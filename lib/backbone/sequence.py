...
class Transformer(nn.Sequential):
	def __init__(self, encoding, **config):
		...
		if encoding == 'drug':
			...
			self.emb = Embeddings(config['input_dim_protein'], config['transformer_emb_size_target'], config['max_len_protein'], config['transformer_dropout_rate'])
	...
	def forward(self, v, pooling=1):
		...
		# return encoded_layers[:,0] # n_batch * seq_len * hidden_dim 
		# print('encoded_layers', encoded_layers.shape)
		if pooling:
			return F.adaptive_avg_pool1d(encoded_layers.transpose(1, 2), output_size=1) # n_batch * hidden_dim
		else:
			return encoded_layers.transpose(1, 2) # n_batch * hidden_dim * seq_len
...
class CNN(nn.Sequential):
	...
	def _forward_features(self, x):
		for l in self.conv:
			x = F.relu(l(x))
		x = F.adaptive_max_pool1d(x, output_size=1) # DeepPurpose.encoders
