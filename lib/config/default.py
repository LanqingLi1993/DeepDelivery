...
config=dict(
    dataset=dict(
        ...
        path='./data', # replace this path with the local path of your datasets
        split=dict(
            ...
            scale=None,
            frac=[0.05, 0.05, 0.9], # fractions of [train, valid, test]
            lt_frac=[0.3, 0.4, 0.3], # fractions of [head, middle, tail]
            open_frac=0.2,
            by_class=True,
            ...
            train_name=None,
            valid_name=None,
            test_name=None, # if all training/validation/test files are specified, load them directly 
        ),
    ),
    backbone=dict(
        ...
        deeppurpose=dict(
            ...
            max_len_drug = 50, # maximum length of drug sequence 
            max_len_protein = 545, # maximum length of protein sequence
    ...
    network=dict(
        pretrained=False, # if true, self-trained model will not be loaded at test
        pretrained_model='',
    ),
    loss=dict(
        type='CrossEntropy', # choose from ['CrossEntropy', 'BalancedSoftmaxCE', 'ClassBalancedCE', 'ClassBalancedFocal', 'CDT', 'CostSensitiveCE', 'CrossEntropyLabelSmooth', 'CrossEntropyLabelAwareSmooth', 'SEQL', 'FocalLoss', 'InfluenceBalancedLoss', 'LDAMLoss', 'DiVEKLD', 'MSE']
    ...
    output_dir='./output', # replace this path with the local path of your model outputs
    name='default',
    seed=1,
    use_gpu=True,
    gpu_id=0,
