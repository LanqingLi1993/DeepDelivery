...
def dgl_collate_func(batch, entity_type):
    ...
    batch = [d for d in batch if all(elem is not None for elem in d["x"])] 
    ...
    if key in ["x", "meta_data"]:
        ...
        for i, entity in enumerate(entity_type):
            if entity == "drug":
                ...
                # if torch.is_tensor(data_lst[i][0][0]): # For pretrained models like ESM, data_lst[i][0]: (input_ids, attention_mask)
                #     data_lst[i] = torch.cat(data_lst[i]).float()
                # else:

                data_lst = torch.tensor(data_lst)
                if data_lst.dtype == torch.float64:
                    data_lst = torch.tensor(data_lst, dtype=torch.float32)
...
class Dataset(torch.utils.data.Dataset):
    ...
    def load(self):
        ...
        for entity in self.entities:
            ...
            self.data[entity + "_raw"] = self.feat_df[entity + "_raw"].to_list()
            self.data[entity + "_ID"] = self.feat_df[entity + "_ID"].to_list() 
    ...
    def featurize(self):
        ...
        for entity, entity_type in zip(self.entities, self.entity_type):
            if entity_type == "drug":
                ...
                self.feat_df[entity + "_raw"] = data_df[entity] # keep track of the raw input       
                if self.drug_encoding == "CNN" or self.drug_encoding == "CNN_RNN":
                    self.feat_df[entity] = self.feat_df[entity].apply(DeepPurpose.utils.drug_2_embed)
                ...
                #                                                 column_name=entity,
                #                                                 save_column_name=entity)
                self.feat_df = utils.featurizer.encode_protein(df_data=self.data_df, 
                                                                target_encoding=self.protein_encoding, 
                                                                column_name=entity,
                                                                save_column_name=entity,
                                                                cfg=self.cfg["backbone"]["deeppurpose"])
                self.feat_df[entity + "_raw"] = data_df[entity] # keep track of the raw input
                if self.protein_encoding == "CNN" or self.protein_encoding == "CNN_RNN":
                    ...
                    self.feat_df[entity] = self.feat_df[entity].apply(utils.utils.protein_2_embed)
    ...
    def __getitem__(self, index):
        ...
        for entity in self.entities:
            ...
            data_raw.append(self.data[entity + "_raw"][index])
            data_id.append(self.data[entity + "_ID"][index])
        ...
        item_dict = {"x": data, "y": label, "y_weight": label_weight, "lt_class": lt_label, "meta": meta,
                       "meta_data": meta_data, "meta_label": meta_label, "x_raw": data_raw, "x_id": data_id}
