...
class ClassBalancedCE(CrossEntropy):
    r"""
    Reference:
    ...
    def __init__(self, para_dict= None):
        super(ClassBalancedCE, self).__init__(para_dict)
        self.beta = self.para_dict['cfg']['loss']['ClassBalancedCE']['BETA']
...
class ClassBalancedFocal(CrossEntropy):
    r"""
    Reference:
    ...
    """
    def __init__(self, para_dict=None):
        super(ClassBalancedFocal, self).__init__(para_dict)
        self.beta = self.para_dict['cfg']['loss']['ClassBalancedFocal']['BETA']
        self.gamma = self.para_dict['cfg']['loss']['ClassBalancedFocal']['GAMMA']
        self.class_balanced_weight = np.array([(1-self.beta)/(1- self.beta ** N) for N in self.num_class_list])
        self.class_balanced_weight = torch.FloatTensor(self.class_balanced_weight / np.sum(self.class_balanced_weight) * self.num_classes).to(self.device)
